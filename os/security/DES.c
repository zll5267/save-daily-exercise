...
//把CHAR转换为INT
static void CharToBit(const unsigned char input[],int output[],int bits){
    int i,j;
    for(j=0;j<8;j++){
        for(i=0;i<8;i++){
...
//把INT转换为CHAR
static void BitToChar(const int intput[], unsigned char output[],int bits){
    int i,j;
    for(j=0;j<8;j++){
        for(i=0;i<8;i++){
...
//子密钥生成
static void subKey_fun(const int input[64],int Subkey[16][48]){//注意输入和输出的位数,int只存放01,密钥为48位16轮
    int loop=1,loop_2=2;
    int i,j;
    int c[28],d[28];
    int pc_1[56]={0};
...
int DES_crypt(des_crypt_mode_t mode,
    ...
    if (data_len % 8 != 0) {
        if (mode != DES_CRYPT_ENCRYPT) {
            printf("in decryption, the input size should be time of 8\n");
            return 0;
        }
        new_data_len = ((data_len >> 3) + 1) << 3; // ((data_len/8) + 1 ) * 8
        new_data = (unsigned char*)malloc(new_data_len);
        if (new_data == NULL) {
            printf("malloc fail!!\n");
            return 0;
        }
        memset(new_data, 0, new_data_len); // padding zero
        memcpy(new_data, data, data_len);
    }
    for (int i = 0; i < data_len; i += 8) {
        if (mode == DES_CRYPT_ENCRYPT) {
            DES_Efun(new_data + i, key, output + i);
        } else if (mode == DES_CRYPT_DECRYPT) {
            DES_Dfun(new_data + i, key, output + i);
        } else {
